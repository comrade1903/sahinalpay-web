...
- **`src/press/` is not the archive**: "Silivri'den…" (`/tr/silivriden`, `/from-silivri`) holds pieces *other people* wrote about Şahin Alpay, transcribed from the İkinci Kitap manuscripts under `Archive/ikinci kitap/`. It has its own types (`src/press/types.ts`), its own page and reader (`src/components/press/`) and its own prerender/sitemap branches. It must never reach `allArchiveItems()`, the home-page counts, the coverage strip or the archive search — `tests/press.test.ts` guards that. The pieces exist in Turkish only, so item routes live under `/tr` alone and `/from-silivri/:slug` redirects to them. `url` is set only where the manuscript itself records the original address; most entries have none and show no source link.
