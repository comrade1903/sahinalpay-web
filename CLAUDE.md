...
- **The home page no longer carries a weekly picks section.** "Benden Seçkiler" / "My Picks" (`App.tsx#WeeklyPicks`, `src/lib/picks.ts`, the `.recent-*` CSS) was removed on 2026-09-16 at the owner's request. The generated summary still carries a `pickPool` per language — nothing reads it now, so drop it from `scripts/generate-archive-summary.mjs` (and its validator/test) if the section is not coming back.
